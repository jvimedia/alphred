...
$url='https://alpha-api.app.net/stream/0/posts/stream/unified?include_deleted=0';
$headers=array(
...
print `/usr/bin/php -q load_avatar.php $username  > /dev/null 2>/dev/null &`;
	...
	$random = rand();
	$w->result('unifiedstream-'.$post->id. '-' .$random, $post->id.' '.$arg, $post->text, "Created " .$fixed. " by " .$user->username. " with " .$source->name, $icon, 'yes', '@'.$username );
}
	echo $w->toxml();
